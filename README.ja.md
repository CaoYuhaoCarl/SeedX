...
| `+ask-strict <本文>` | 保存後、元メッセージを block。`+start` 送信後にオーケストレーター起動 | 2 ステップ |
| `+start [path]` | 指定パスまたは最新の質問ファイルから起動 | — |

クリップボードと strict は同じ隔離強度です（本文はメイン Agent の context に一切入らない）。違いは UX のみ。隔離契約の詳細は [CLAUDE.md §1.2](CLAUDE.md) を参照してください。
